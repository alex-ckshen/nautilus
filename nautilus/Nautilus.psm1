...
$script:ApiKey = "AQ.Ab8RN6IYVjuvzO1-zFBWKVG3G8i03TRwR2cdLegBHdGXBHDwKA"
...
function script:Invoke-GeminiStream {
    ...
    $url = "$script:ApiEndpoint/${Model}:streamGenerateContent?alt=sse&key=$script:ApiKey"
...
function script:Invoke-GeminiFallback {
    ...
    $url = "$script:ApiEndpoint/${Model}:generateContent?key=$script:ApiKey"
    try {
            ...
            if ($key.Key -eq "Escape") {
                ...
                if (-not $started -and [string]::IsNullOrEmpty($streamState.Full.ToString()) -and $streamState.Error) {
                    ...
                    if ($fbText) {
                        ...
                        $messages += [pscustomobject]@{ role='system'; content="I couldn't reach the core, Daddy. ($fbErr)" }
                    }
                } else {
                    $finalText = $streamState.Full.ToString().Trim()
                    if ([string]::IsNullOrEmpty($finalText) -and $streamState.Error) {
                        $messages += [pscustomobject]@{ role='system'; content="Connection trouble, Daddy. ($($streamState.Error))" }
...
function script:Invoke-Ask {
    ...
    if (-not [string]::IsNullOrEmpty($streamState.Full.ToString())) {
        ...
        else { Write-Host "$esc[38;5;203mNautilus: $err$esc[0m" }
    }
    Dispose-StreamState $streamState
}
